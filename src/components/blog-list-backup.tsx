...
// Demo blog verileri (fallback için)
...
export function BlogList() {
  ...
  const loadBlogs = async () => {
    try {
      ...
      const result = await getBlogs(filters, { page: 1, limit: 1000 }) // Tüm blogları getir
  ...
  const confirmDeleteBlog = async () => {
    ...
    try {
      ...
      showToast("Blog silinirken bir hata oluştu", "error")
    } finally {
      setIsDeleting(false)
    }
  }
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="relative w-full sm:w-80 lg:w-96">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-neutral-300 z-10" />
            <input
              type="text"
              placeholder="Blog ara..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-3 glass rounded-xl text-white placeholder-neutral-400 focus:outline-none focus:ring-2 focus:ring-cyan-500/70 transition-all duration-300 backdrop-blur-lg border border-white/20"
          ...
          <button
            ...
            className="flex items-center space-x-2 px-4 py-3 glass rounded-xl text-white hover:bg-white/10 transition-all duration-200 border border-white/20"
            style={{ background: 'rgba(255, 255, 255, 0.1)' }}
          >
        ...
        {isSelectMode && (
          <div className="flex flex-wrap items-center gap-2 p-4 glass rounded-xl border border-white/20" style={{ background: 'rgba(255, 255, 255, 0.05)' }}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <Link
            ...
            className="flex items-center space-x-2 px-4 py-3 bg-gradient-to-r from-cyan-500 to-blue-500 text-white rounded-xl hover:from-cyan-600 hover:to-blue-600 transition-all duration-200 shadow-lg hover:shadow-xl"
          >
      ...
      <AnimatePresence>
        {showFilters && (
          <motion.div
            ...
            className="glass rounded-xl p-6 border border-white/20 backdrop-blur-lg overflow-hidden"
            style={{ background: 'rgba(255, 255, 255, 0.1)' }}
          >
          ...
          <select
            ...
            onChange={(e) => setSelectedCategory(e.target.value)}
                  className="w-full px-4 py-3 glass rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/70 transition-all duration-300 appearance-none cursor-pointer border border-white/20"
                  style={{ background: 'rgba(255, 255, 255, 0.1)' }}
          >
          ...
          >
            {statuses.map(status => (
              ...
              <div className="flex items-end">
                <button
                  ...
                  className="w-full px-4 py-3 text-neutral-400 hover:text-white hover:bg-white/10 rounded-xl transition-all duration-200 border border-white/20"
                >
                  Filtreleri Temizle
                </button>
        </div>
            ...
            {/* Tags Filter */}
            <div className="mt-4">
              <label className="block text-sm font-medium text-white mb-3">Etiketler</label>
                  ...
                  <button
                    ...
                    className={`px-3 py-2 rounded-lg text-sm transition-all duration-200 ${
                      selectedTags.includes(tag)
                        ...
                        : "bg-white/10 text-neutral-400 hover:text-white hover:bg-white/20 border border-white/20"
                    }`}
                  >
                    {tag}
                  </button>
        ...
        <div className={viewMode === "grid" ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" : "space-y-4"}>
          ...
          {getCurrentPageBlogs().map((blog, index) => (
            ...
            >
              ...
              {/* Blog Content Link */}
              <Link
                href={`/content-management-system-2024/blogs/${blog.id}?from=${encodeURIComponent(window.location.pathname)}`}
              ...
              <div className="mb-6 p-4 bg-white/5 rounded-lg border border-white/10">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-medium text-white line-clamp-2 mb-1">{blogToDelete.title}</h4>
                    <p className="text-sm text-neutral-400 line-clamp-2 mb-2">{blogToDelete.excerpt}</p>
                    <div className="flex items-center space-x-4 text-xs text-neutral-500">
                      <span className="flex items-center space-x-1">
                        <User className="h-3 w-3" />
                        <span>{blogToDelete.author}</span>
                      </span>
                      <span className="flex items-center space-x-1">
                        <Calendar className="h-3 w-3" />
                        <span>
                          {blogToDelete.createdAt ? (
                            blogToDelete.createdAt.toDate ? 
                              blogToDelete.createdAt.toDate().toLocaleDateString('tr-TR') : 
                              new Date(blogToDelete.createdAt).toLocaleDateString('tr-TR')
                          ) : 'Tarih yok'}
                        </span>
                      </span>
                    </div>
